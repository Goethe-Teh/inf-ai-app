// force rebuild: 20250421-settings-v2
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const storedSetup = JSON.parse(localStorage.getItem('infinity_setup')) || {};
    const storedPasscode = localStorage.getItem('infinity_passcode') || '';

    setUsername(storedUser);
    setNewUsername(storedUser);
    setSetupData(storedSetup);
  ...
  return (
    <div style={{ padding: 30, maxWidth: 600, margin: 'auto' }}>
      ...
      <input
        ...
        style={{ width: '100%', marginBottom: 10 }}
      />
      <button onClick={handleSaveAccount} style={{ width: '100%', padding: 10, marginBottom: 20 }}>
      ...
      <input
        ...
        style={{ width: '100%', marginBottom: 10 }}
      />
      <button onClick={handleSavePasscode} style={{ width: '100%', padding: 10, marginBottom: 20 }}>
        ตั้งรหัสผ่าน
      </button>

      <h4>ข้อมูล AI ที่คุณตั้งไว้</h4>
      <pre style={{ background: '#f5f5f5', padding: 10 }}>
        {JSON.stringify(setupData, null, 2)}
      </pre>

      <button onClick={handleBack} style={{ width: '100%', padding: 10, backgroundColor: '#eee' }}>
        กลับสู่ห้อง Infinity Chat
      </button>
